...
task_status_t wifi_connect(task_node_t *task_node)
{
    ...
    switch (self->state)
    {
        ...
        case WIFI_STATE_RECONNECT:
        {
            ...
            {
                ESP_LOGW(self->tag, "Max retries (%d) reached — waiting for a new connection request",
                         REDMOLE_MAX_RETRY);
                self->retry_count = 0;
                self->state       = WIFI_STATE_IDLE;
                return TASK_DONE;
            ...
            ESP_LOGI(self->tag, "esp_wifi_set_config succeeded with ssid '%s'", ssid);
...
void nac_connect_to_saved_wifi(const char *ssid, const char *password)
{
    if (!ssid || ssid[0] == '\0')
    {
        ESP_LOGW("NAC", "Cannot connect without a saved SSID");
        return;
    }

    /*
     * Do not gate connection on a scan result. Hidden or temporarily missed
     * APs are valid connection targets. The asynchronous connection state
     * machine owns retries and reports the actual association failure reason.
     */
    esp_err_t err = nac_request_wifi_connect(ssid, password ? password : "");
    if (err != ESP_OK)
    {
        ESP_LOGE("NAC", "Failed to queue saved WiFi connection: %s", esp_err_to_name(err));
    }
}
