 ...
 *
 * This layer exposes screen operations to the GUI runtime while delegating
 * LVGL object ownership to gui_view_t.
 */
 ...
 *
 * @param screen Screen object to initialize, must not be NULL.
 * @param model Initial model applied while creating the screen, must not be NULL.
 * @param nav_event_cb Navigation callback for panel switching actions, may be NULL.
 * @param settings_event_cb Settings callback for controls owned by the settings panel, may be NULL.
 * @param event_user_data Caller-owned user data forwarded to the event callbacks.
 */
 ...
 *
 * @param screen Initialized screen object, must not be NULL.
 ...
 *
 * @param screen Initialized screen object; NULL or missing brightness widgets are ignored.
